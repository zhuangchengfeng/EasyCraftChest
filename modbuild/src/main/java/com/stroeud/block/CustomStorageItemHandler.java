...
public final class CustomStorageItemHandler
implements IItemHandler {
    private static final int MAX_AUTOMATION_SLOTS = 54;
    private final Mode mode;
    private final ServerLevel level;
    private final BlockPos pos;
    private final CustomStorageManager manager;
    ...
    public CustomStorageItemHandler(Mode mode, ServerLevel level, BlockPos pos) {
        this.mode = mode;
        this.level = level;
        this.pos = pos;
        this.manager = CustomStorageManager.get(level);
    }

    private void ensureIndex(CustomStorageData storage) {
    ...
    private ItemStack templateFor(CustomStorageData storage, String itemKey) {
    ...
    public int getSlots() {
        ...
        CustomStorageData storage = this.manager.getStorage(this.pos);
        if (storage == null) {
            return 1;
        }
    ...
    public ItemStack insertItem(int slot, @Nonnull ItemStack stack, boolean simulate) {
        ...
        CustomStorageData storage = this.manager.getOrCreateStorage(this.pos, this.level);
        if (storage == null) {
            return stack;
        }
        if (!simulate) {
