package com.easycraftchest.config;

import net.neoforged.neoforge.common.ModConfigSpec;

/**
 * 模组 TOML 配置。注册后生成到 config/easycraftchest.toml。
 * 相关参数可在游戏内/文件里调整并热重载(服务端类型)。
 */
public final class ModConfigs {
    public static final ModConfigSpec.Builder BUILDER = new ModConfigSpec.Builder();

    /** 配置版本标记:升级导致默认值/新增项不兼容旧配置时,请 +1。加载前会检测,版本不符就删除重建。 */
    public static final int CONFIG_VERSION = 1;

    public static final ModConfigSpec.IntValue SYNTHESIS_TIMEOUT_MILLIS = BUILDER
        .comment(
            "easycraftchest-config-version=" + ModConfigs.CONFIG_VERSION,
            "(改版本号即强制重建该配置文件;升级大改后旧的默认值不生效)",
            "一键合成配方解析的超时时间(毫秒)。",
